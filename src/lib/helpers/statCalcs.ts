...
export const getAvg = (stats: any) => (stats?.summary_at_bats === 0) ? 0 : stats?.summary_hits / stats?.summary_at_bats;
export const getObp = (stats: any) => (getPa(stats) === 0) ? 0 : (stats?.summary_hits + stats?.summary_walks_hbp + stats?.summary_walks_bb) / getPa(stats).toFixed(3);
export const getSlg = (stats: any) => (stats?.summary_at_bats === 0) ? 0 : (stats?.summary_singles + (stats?.summary_doubles * 2) + (stats?.summary_triples * 3) + (stats?.summary_homeruns * 4)) / stats?.summary_at_bats
...
export const getERA = (stats: any) => (stats?.outs_pitched === 0) ? ((stats?.runs_allowed === 0) ? 0.00 : 99.99) : 
